...
class ProfitHarvester:
    ...
    def __init__(self, bot: Any = None, exchange: Any = None, min_profit_pct: float = 1.0,
        ...
        # Ultra-aggressive profit-taking for maximum compounding
        self.profit_levels = config.get('profit_levels', [
            {'threshold': 0.08, 'sell_pct': 100, 'confidence': 0.95},  # 0.08% profit - ultra-micro scalping
            {'threshold': 0.15, 'sell_pct': 100, 'confidence': 0.92},  # 0.15% profit - micro scalping
            {'threshold': 0.25, 'sell_pct': 100, 'confidence': 0.90},  # 0.25% profit - mini scalping
            {'threshold': 0.5, 'sell_pct': 100, 'confidence': 0.88}    # 0.5% profit - standard scalping
        ]) if config else [
            {'threshold': 0.08, 'sell_pct': 100, 'confidence': 0.95},
            {'threshold': 0.15, 'sell_pct': 100, 'confidence': 0.92},
            {'threshold': 0.25, 'sell_pct': 100, 'confidence': 0.90},
            {'threshold': 0.5, 'sell_pct': 100, 'confidence': 0.88}
        ]

        # Ultra-aggressive settings for micro-profit harvesting
        self.target_profit_pct = config.get('target_profit_pct', 0.15) if config else 0.15  # 0.15% target
        self.stop_loss_pct = config.get('stop_loss_pct', 0.3) if config else 0.3  # 0.3% stop loss

        # Micro-profit optimization settings
        self.ultra_micro_threshold = 0.08  # 0.08% minimum profit to harvest
        self.compound_reinvestment_pct = 98  # Reinvest 98% of all profits immediately
        self.progressive_selling = config.get('progressive_selling', True) if config else True

        self.logger.info("[HARVESTER] Enhanced ProfitHarvester initialized:")
    ...
    async def track_position(self, symbol: str, entry_price: float, amount: float,
        ...
        try:
            ...
            if hasattr(self, 'bot') and self.bot and hasattr(self.bot, 'portfolio_tracker'):
                ...
                self.logger.warning("[HARVESTER] No portfolio tracker available - position tracking may be incomplete")
                ...
                for position in positions:
                    ...
                    if entry_price == 0 and current_price > 0 and amount > 0:
                        ...
                        if self.progressive_selling:
                            ...
                            # Ultra-aggressive micro-profit harvesting for maximum compounding
                            if profit_pct >= 0.08:  # Any profit >= 0.08%
                                should_sell = True
                                if profit_pct < 0.1:
                                    sell_reason = f"Ultra-micro-profit harvest: {profit_pct:.3f}%"
                                    sell_confidence = 0.98  # Extremely high confidence for tiny profits
                                elif profit_pct < 0.3:
                                    sell_reason = f"Micro-profit harvest: {profit_pct:.3f}%"
                                    sell_confidence = 0.95
                                else:
                                    sell_reason = f"Standard profit harvest: {profit_pct:.3f}%"
                                    sell_confidence = 0.92
        ...
        try:
            ...
            if hasattr(self, 'bot') and self.bot and hasattr(self.bot, 'balance_manager'):
                try:
                    ...
                    self.logger.info("[HARVESTER] [OK] Balance refreshed after sell completion")
                except Exception as e:
    ...
    async def _trigger_reinvestment(self, amount_to_reinvest: float, profit_usdt: float, is_tier_1: bool = False) -> dict[str, Any]:
        ...
        try:
            ...
            if reinvestment_symbols:
                ...
                reinvestment_signal = {
                    ...
                    'metadata': {
                        ...
                        'is_tier_1': is_tier_1,
                        'reason': 'Seamless reinvestment from profit harvesting'
                    }
                }
            ...
            self.logger.info("[HARVESTER] No immediate opportunities found, profit will be available for next scan cycle")
            return {
