...
class UnifiedRiskManager:
    ...
    async def validate_trade(self, symbol: str, side: str, amount: float, price: float = None) -> Tuple[bool, str, Dict[str, Any]]:
        ...
        async with self._lock:
            ...
            if balance <= 0 and not is_exit_trade:
            ...
            elif balance <= 0 and is_exit_trade and portfolio_value > 0:
            ...
            elif balance <= 0 and portfolio_value <= 0:
            ...
            daily_loss_limit = (effective_balance * self.risk_params['max_daily_loss'] / 100)
    ...
    async def add_position(self, symbol: str, size: float, entry_price: float, side: str = 'buy'):
        ...
        async with self._lock:
            position = PositionRisk(
                ...
                unrealized_pnl=0.0,
                risk_amount=size * self.risk_params['stop_loss_pct'],
                stop_loss=entry_price * (1 - self.risk_params['stop_loss_pct']) if side == 'buy' else None,
                take_profit=entry_price * (1 + self.risk_params['take_profit_pct']) if side == 'buy' else None,
    ...
    async def update_position(self, symbol: str, current_price: float):
        ...
        async with self._lock:
            ...
            if self.risk_params['use_trailing_stops'] and position.unrealized_pnl > 0:
    ...
    async def _get_available_balance(self) -> float:
        ...
        if self.balance_manager:
            return await self.balance_manager.get_balance_for_asset('USDT')
        return 0.0
    ...
    async def _get_portfolio_value(self) -> float:
        ...
        try:
            ...
            total_value = usdt_balance + unrealized_pnl + held_value
