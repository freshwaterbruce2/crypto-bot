...
class KrakenAuth:
    ...
    def get_auth_headers(
        ...
        try:
            ...
            nonce = self.nonce_manager.get_nonce("kraken_auth")
    ...
    async def get_auth_headers_async(
        ...
        async with self._auth_lock:
            ...
            try:
                # Get fresh nonce from unified manager (async)
                nonce = await self.nonce_manager.get_nonce_async("kraken_auth")
    ...
    def _handle_nonce_error(
        ...
        try:
            # Get recovery nonce from unified manager
            recovery_nonce = self.nonce_manager.handle_invalid_nonce_error("kraken_auth")
            ...
            logger.info(f"[KRAKEN_AUTH_2025] Nonce error recovery successful")
            return headers
    ...
    def _handle_signature_error(
        ...
        try:
            # Get fresh nonce from unified manager and regenerate signature
            fresh_nonce = self.nonce_manager.get_nonce("kraken_auth")
            ...
            logger.info(f"[KRAKEN_AUTH_2025] Signature error recovery successful")
            return headers
    ...
    def export_configuration(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'nonce_manager_config': {
                'type': 'UnifiedKrakenNonceManager',
