...
class CredentialManager:
    ...
    def get_kraken_credentials(self) -> tuple[Optional[str], Optional[str]]:
        ...
        if (not api_key or not private_key) and WSL_ENVIRONMENT and WINDOWS_ENV_BRIDGE_AVAILABLE:
            ...
            try:
                ...
                if windows_api_key and windows_private_key:
                    ...
                    logger.info(
                        "Successfully loaded credentials from Windows environment variables"
                    )
                else:
                    # Try legacy Windows environment variable names
                    windows_api_key = get_windows_env_var("KRAKEN_API_KEY") or get_windows_env_var(
                        "API_KEY"
                    )
                    windows_private_key = get_windows_env_var(
                        "KRAKEN_API_SECRET"
                    ) or get_windows_env_var("API_SECRET")
                    ...
                    if windows_api_key and windows_private_key:
                        ...
                        logger.info(
                            "Successfully loaded credentials from Windows environment variables (legacy names)"
                        )
        ...
        if api_key and private_key:
            ...
            if WSL_ENVIRONMENT and WINDOWS_ENV_BRIDGE_AVAILABLE:
                logger.warning(
                    "Kraken API credentials not found in Linux environment variables or Windows environment variables"
                )
            else:
    ...
    def get_kraken_rest_credentials(self) -> tuple[Optional[str], Optional[str]]:
        ...
        if not api_key or not private_key:
            ...
            private_key = os.getenv("KRAKEN_API_SECRET") or os.getenv("API_SECRET")
            credential_source = (
                "legacy generic (KRAKEN_API_KEY/KRAKEN_API_SECRET or API_KEY/API_SECRET)"
            )
        ...
        if api_key and private_key:
            logger.info(
                f"Kraken REST API credentials loaded from environment using {credential_source}"
            )
            return api_key, private_key
        else:
            if WSL_ENVIRONMENT and WINDOWS_ENV_BRIDGE_AVAILABLE:
                logger.warning(
                    "Kraken REST API credentials not found in Linux environment variables or Windows environment variables"
                )
            else:
    ...
    def get_kraken_websocket_credentials(self) -> tuple[Optional[str], Optional[str]]:
        ...
        if not api_key or not private_key:
            ...
            credential_source = (
                "WebSocket-specific (KRAKEN_WEBSOCKET_API_KEY/KRAKEN_WEBSOCKET_API_SECRET)"
            )
        ...
        if (not api_key or not private_key) and WSL_ENVIRONMENT and WINDOWS_ENV_BRIDGE_AVAILABLE:
            try:
                ...
                logger.warning(
                    f"Failed to load WebSocket credentials from Windows environment: {e}"
                )

        if api_key and private_key:
            logger.info(
                f"Kraken WebSocket API credentials loaded from environment using {credential_source}"
            )
            return api_key, private_key
        else:
            if WSL_ENVIRONMENT and WINDOWS_ENV_BRIDGE_AVAILABLE:
                logger.warning(
                    "Kraken WebSocket API credentials not found in Linux environment variables or Windows environment variables"
                )
            else:
                logger.warning(
                    "Kraken WebSocket API credentials not found in environment variables"
                )
            return None, None
    ...
    def get_credential_status(self) -> dict[str, Any]:
        ...
        status = {
            ...
            "unified_credentials": {
                ...
                "valid": self.validate_credentials(unified_key, unified_secret)
                if unified_key and unified_secret
                else False,
                "api_key_preview": f"{unified_key[:8]}..."
                if unified_key and len(unified_key) > 8
                else "***",
            ...
            "generic_credentials": {
                ...
                "valid": self.validate_credentials(api_key, private_key)
                if api_key and private_key
                else False,
            ...
            "rest_credentials": {
                ...
                "valid": self.validate_credentials(rest_api_key, rest_private_key)
                if rest_api_key and rest_private_key
                else False,
                "api_key_preview": f"{rest_api_key[:8]}..."
                if rest_api_key and len(rest_api_key) > 8
                else "***",
                "using_unified": unified_key is not None and unified_secret is not None,
                "source": "unified" if (unified_key and unified_secret) else "legacy",
            },
            "websocket_credentials": {
                ...
                "valid": self.validate_credentials(ws_api_key, ws_private_key)
                if ws_api_key and ws_private_key
                else False,
                "api_key_preview": f"{ws_api_key[:8]}..."
                if ws_api_key and len(ws_api_key) > 8
                else "***",
                "using_unified": unified_key is not None and unified_secret is not None,
                "source": "unified" if (unified_key and unified_secret) else "legacy",
            },
            ...
            "api_key_present": api_key is not None,
            "private_key_present": private_key is not None,
            "credentials_valid": self.validate_credentials(api_key, private_key)
            if api_key and private_key
            else False,
            # Migration info
            "migration_info": {
                ...
                "should_migrate": unified_key is None
                and unified_secret is None
