...
class RealtimePnLTracker:
    ...
    async def _calculate_portfolio_value(self) -> float:
        ...
        try:
            ...
            with open('trading_data/portfolio_state.json') as f:
            ...
            for symbol, data in portfolio.items():
                ...
                if isinstance(data, dict):
                    ...
                    logger.debug(f"[PNL_TRACKER] Invalid portfolio data type for {symbol}: {type(data)}")
        ...
        try:
            ...
            for symbol, data in portfolio.items():
                ...
                if isinstance(data, dict):
                    ...
                    logger.debug(f"[PNL_TRACKER] Skipping invalid portfolio data for {symbol}: {type(data)}")
    ...
    async def _check_position_profit(self, symbol: str, position_data: Dict[str, Any]):
        ...
        try:
            ...
            if not isinstance(position_data, dict):
                logger.debug(f"[PNL_TRACKER] Invalid position data type for {symbol}: {type(position_data)}")
                return
