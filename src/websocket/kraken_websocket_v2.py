...
from .data_models import (
    BalanceUpdate, TickerUpdate, OrderBookUpdate, TradeUpdate, OHLCUpdate,
    SubscriptionRequest, SubscriptionResponse, ConnectionStatus
)
...
class KrakenWebSocketConfig:
    ...
    # Authentication
    token_refresh_interval: float = 13 * 60  # 13 minutes (2 min before expiry)
    ...
    async def _token_refresh_loop(self):
        ...
        while self.is_running and self.private_connection and self.private_connection.is_connected:
            try:
                ...
                logger.info("[KRAKEN_WS_V2] Refreshing authentication token")
                
                # Get new token
                if await self._get_auth_token():
                    logger.info("[KRAKEN_WS_V2] Token refreshed successfully")
                else:
                    logger.error("[KRAKEN_WS_V2] Token refresh failed")
